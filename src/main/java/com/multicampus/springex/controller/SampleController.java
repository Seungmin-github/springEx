...
@Controller //스프링MVC에서 컨트롤러 역할 , 스프링의 빈(Bean)으로 등록처리하겠다는 설정
@Log4j2
public class SampleController {
    @GetMapping("/hello") //Get방식으로 들어오는 요청을 처리하기 위한 애너테이션
    public void hello(){
        log.info("hello~");
    }
    ...
    @GetMapping("/ex5")
    public String ex5(RedirectAttributes redirectAttributes) { //객체 자동생성
        redirectAttributes.addAttribute("name", "ABCCCDD");
        redirectAttributes.addFlashAttribute("result", "success");
        return "redirect:/ex6";
        //redirectAttributes.addFlashAttribute 처음 한번만 출력
    }
